 ...
 * DECLARE @StartDate / @EndDate / @AsOfDate blocks are intentionally omitted
 * here; the query runner substitutes those parameters at execution time.
 *
...
SELECT
    ...
    DATEDIFF(DAY, @StartDate, @EndDate) + 1 AS days_in_window,
    CAST(SUM(CASE WHEN patient_days > 0 THEN patient_days ELSE 0 END) * 1.0
         / (DATEDIFF(DAY, @StartDate, @EndDate) + 1)
         AS decimal(10,2)) AS implied_adc
FROM overlap
GROUP BY bucket
ORDER BY bucket_sort, bucket;`,
...
(
    SELECT
        ...
        pd.patient_days
    FROM revenue r
    FULL OUTER JOIN patient_days pd ON pd.bucket = r.bucket
)
SELECT
    ISNULL(bucket, '(unmapped)') AS bucket,
    bucket_sort,
...
SELECT
    ...
    SUM(
        CASE WHEN cleared_by_sla = 1 THEN c.li_calculatedamount ELSE 0 END
    ) AS cleared_amount
FROM classified c
LEFT JOIN dim_branch db ON db.epi_branchcode = c.branch_code
...
lupa_codes AS
(
    SELECT * FROM (VALUES ('LUPA'), ('L')) AS c(code)
),
periods AS
(
    SELECT
...
INNER JOIN dbo.SERVICECODES sc ON sc.sc_id = v.CEV_SC_ID
WHERE v.cev_deleted = 0
  AND CAST(v.CEV_VISITDATE AS date) BETWEEN @StartDate AND @EndDate
GROUP BY v.CEV_AGID;
...
WHERE wpd_active = 'Y'
  ...
  AND (wpd_effectiveto IS NULL OR wpd_effectiveto >= @StartDate);
...
INNER JOIN #expected ex ON ex.wkr_id = w.wkr_id AND ex.rn = 1
LEFT JOIN dbo.WORKER_HOMEBRANCH wh ON wh.wh_wkrid = w.wkr_id
LEFT JOIN #earned e ON e.wkr_id = w.wkr_id
...
SELECT
    *,
    ...
    CASE
        WHEN earned_points >= COALESCE(NULLIF(expected_points,0), 30.0)
        THEN 1
        ELSE 0
    END AS is_achieving
INTO #scored
...
SELECT
    ...
    CAST(
        100.0 * SUM(s.is_achieving) / NULLIF(COUNT(*),0)
        AS decimal(6,2)
    ) AS pct_achieving,
    SUM(s.visit_count) AS visits,
    SUM(s.earned_points) AS earned_points
FROM #scored s
LEFT JOIN #bucket_map bm ON bm.branch_code = s.branch_code
GROUP BY bm.bucket, s.branch_code
ORDER BY MIN(bm.bucket_sort), s.branch_code;
...
FROM #scored s
LEFT JOIN #bucket_map bm ON bm.branch_code = s.branch_code
ORDER BY bm.bucket_sort, s.wkr_fullname;`,
...
episodes AS (
    SELECT
        e.epi_id, e.epi_SocDate, e.epi_DischargeDate, e.epi_NonAdmitDate,
        d.service_line, d.branch_name
    FROM dbo.CLIENT_EPISODES_ALL e
    LEFT JOIN dim_branch d
        ON d.epi_slid = e.epi_slid
       AND RTRIM(d.epi_branchcode) = RTRIM(e.epi_branchcode)
...
census AS (
    SELECT service_line, branch_name, COUNT(*) AS current_census
    FROM episodes
    WHERE epi_SocDate <= @AsOfDate
      AND (epi_DischargeDate IS NULL OR epi_DischargeDate > @AsOfDate)
    GROUP BY service_line, branch_name
...
hh_pal AS (
    SELECT SUM(current_census) AS raw_hh_pal_census
    FROM census
...
FROM census;`,
  ...
  // ── Visit Notes (Client Episode Visit Notes with Service Lines) ───────────
...
kpi_calc AS (
    ...
    FROM admissions a
    LEFT JOIN bp1_visits b ON a.epi_id = b.epi_id
)
SELECT
...
FROM ar
LEFT JOIN dim_branch db ON db.epi_branchcode = ar.branch_code
...
FROM unbilled u
LEFT JOIN dim_branch db ON db.epi_branchcode = u.branch_code
...
(
    ...
    FROM episodes ep
    LEFT JOIN NOE_DELAY_REASON noe ON noe.ndr_id = ep.epi_id
    LEFT JOIN NONCOVERED_REASON nc ON nc.ncr_id = ep.epi_id
    LEFT JOIN FACETOFACE_STATUSES f2f ON f2f.ftfs_id = ep.epi_id
    LEFT JOIN CLIENT_EPISODE_ADMISSION_TYPES cert ON cert.ceat_epiid = ep.epi_id
    LEFT JOIN CLIENT_EPISODE_RECERT_HISTORY rec ON rec.cerh_epiid = ep.epi_id
),
scored AS
(
    SELECT *,
...
ORDER BY bucket_sort, bucket;`,
  ...
  // ── Referrals and NTUC ────────────────────────────────────────────────────
  referrals_ntuc: `;WITH bucket_map (sl_name, sl_id, branch_code, bucket) AS
(
...
FROM nonadmits na
LEFT JOIN dbo.NONADMIT_REASONS nac ON nac.nac_code = na.epi_NonAdmitCode
GROUP BY
    na.bucket,
    na.epi_NonAdmitCode,
    nac.nac_desc,
