...
export function SemanticSearchPanel({ onSelectTable, initialQuery = "" }: SemanticSearchPanelProps) {
  ...
  return <div className="flex flex-col gap-4">
    ...
    {searched && !loading && <div className="flex items-center justify-between text-xs text-muted-foreground"><span>{results.length} matching assets</span><span>{backend === "pgvector-hybrid" ? "Semantic + keyword ranking" : "Catalog keyword ranking"}</span></div>}
    <div className="grid gap-2">{results.map((result) => { const Icon = icons[result.type] ?? FileText; return <button type="button" key={result.id} onClick={() => result.type === "table" && onSelectTable?.(result.id)} className="flex items-start gap-3 rounded-xl border border-border bg-card p-3 text-left transition hover:border-primary/40 hover:bg-accent/30"><span className="rounded-lg bg-primary/10 p-2 text-primary"><Icon className="size-4" /></span><span className="min-w-0 flex-1"><span className="flex items-center gap-2"><strong className="truncate text-sm text-foreground">{result.name}</strong><span className="rounded border border-border px-1.5 py-0.5 text-[10px] uppercase text-muted-foreground">{result.type}</span><span className="ml-auto font-mono text-[10px] text-primary">{Math.round(result.score * 100)}%</span></span><span className="mt-1 block text-xs leading-relaxed text-muted-foreground">{result.description}</span><span className="mt-2 block text-[10px] text-muted-foreground">{result.location}</span></span><ChevronRight className="mt-2 size-4 shrink-0 text-muted-foreground" /></button> })}</div>
