...
export function MetadataReportEngine() {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Model summary bar */}
      <ModelSummaryBar model={model} onReset={() => { setCachedModel(null as unknown as SchemaModel); setModel(null); }} />
...
function MetadataUploader({ onModelReady }: { onModelReady: (m: SchemaModel) => void }) {
  ...
  async function processFile(file: File) {
    ...
    try {
      ...
      const tables = parseMetadata(raw);
      if (tables.length === 0) throw new Error("No tables found. Check your metadata.json format.");
      const model = transformToSchemaModel(tables, raw);
      onModelReady(model);
    } catch (e) {
    ...
    <div className="flex flex-col gap-5 max-w-2xl">
      <div>
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          Upload your <code className="font-mono text-primary">metadata.json</code> to generate a
          full schema intelligence model — including column roles, domain grouping, join paths, and
          field discovery. The engine uses only metadata; no live DB queries are made.
        </p>
      </div>
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            or click to browse · supports <code className="font-mono">tables[]</code>,{" "}
            <code className="font-mono">schemas{"{}"}</code>, or combined format
          </p>
        </div>
      </div>
      ...
      {open && (
        <div className="border-t border-border p-3 flex flex-col gap-2">
          <textarea
            ...
            placeholder='{"tables": [{"name": "ORDERS", "schema": "dbo", ...}]}'
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 p-3 bg-card border border-border rounded-lg">
      <div className="flex flex-wrap items-center gap-4">
        ...
        <StatChip label="Domains" value={domainCount} color="chart-3" />
        <StatChip label="Relationships" value={edgeCount} color="chart-2" />
        <StatChip label="Index terms" value={indexSize} color="chart-4" />
        <span className="text-[10px] text-muted-foreground font-mono hidden sm:inline">
          hash: {model.sourceHash}
        </span>
      </div>
      <button
        onClick={onReset}
...
function StatChip({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="flex items-center gap-1.5">
      <span className={`text-sm font-semibold text-${color}`}>{value}</span>
      ...
      {expanded && (
        <div className="divide-y divide-border/40">
          {tableIds.map((id) => {
            ...
            const selected = selectedTableIds.includes(id);
            ...
            return (
              <button
                ...
                className={cn(
                  "flex items-start justify-between w-full px-3 py-2.5 text-left transition-colors",
                  selected
  ...
  return (
    <div className="p-4 flex flex-col gap-4">
      ...
      <FieldSection
        ...
        emptyText="No numeric measure columns found in selected tables"
...
function JoinVisualizer({
  ...
  // Build graph positions for selected tables
  const allEdges = model.graph.edges.filter(
    (e) => selectedTableIds.includes(e.from) && selectedTableIds.includes(e.to) && e.type === "many-to-one"
  );
  ...
  return (
    <div className="p-4 flex flex-col gap-4">
          ...
          {selectedTableIds.map((id, idx) => {
            ...
            return (
              <div key={id} className="flex items-center gap-3">
                <div
                  className={cn(
                    "border rounded-md px-3 py-2.5 min-w-[130px]",
                    isRoot
                ...
                {outEdges.map((edge) => (
                  <div key={`${edge.from}-${edge.to}`} className="flex items-center gap-1">
                    <div className="flex flex-col items-center">
                      <div className="h-px w-8 bg-chart-2/50" />
                      <span className="text-[9px] text-chart-2 font-mono">{edge.via}</span>
                      <span className="text-[9px] text-muted-foreground">{edge.type}</span>
                    </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-1">
            {resolvedJoins.map((join, i) => (
              <div
                key={i}
                className="flex items-center gap-2 px-3 py-2 bg-muted/30 border border-border/50 rounded-md"
              >
                ...
                <code className="text-[11px] font-mono text-primary">
                  {join.condition ?? `${join.from}.${join.via} = ${join.to}.id`}
                </code>
              </div>
            ))}
          </div>
        ...
        selectedTableIds.length > 1 && (
          <div className="flex items-start gap-2 p-3 rounded-md bg-chart-5/10 border border-chart-5/20">
            ...
            <p className="text-xs text-chart-5">
              No direct join path found between the selected tables. The tables may require an
      ...
      {/* All available join paths from model */}
      ...
      {open && (
        <div className="border-t border-border p-3 flex flex-col gap-2">
          {relevant.map(([src, paths]) => (
            <div key={src}>
              <p className="text-[10px] font-mono font-semibold text-primary mb-1">{src}</p>
...
function QueryBuilderPanel({
  ...
  function addFilter() {
    ...
    onFiltersChange([
      ...activeFilters,
      {
        table: firstDef.table,
        column: firstDef.column,
        operator: "=",
        value: "",
      },
    ]);
  ...
  function updateFilter(idx: number, patch: Partial<ReportFilter>) {
    onFiltersChange(
      activeFilters.map((f, i) => (i === idx ? { ...f, ...patch } : f))
    );
  }
  ...
  return (
    <div className="p-4 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <SummaryTile label="Measures" items={selectedMeasures.map((m) => m.column)} color="chart-3" />
        <SummaryTile label="Dimensions" items={selectedDimensions.map((d) => d.column)} color="chart-2" />
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        {activeFilters.length === 0 ? (
          <p className="text-[11px] text-muted-foreground">
            No filters applied. Click "Add filter" to restrict results.
          </p>
        ) : (
      ...
      {reportPlan && (
        <div className="flex flex-col gap-2">
          ...
          <pre className="text-[11px] font-mono text-foreground/80 bg-muted border border-border rounded-md p-3 overflow-x-auto max-h-72 leading-relaxed">
  ...
  return (
    <div className="border border-border rounded-md p-2.5 bg-muted/20">
      ...
      {items.length === 0 ? (
        ...
        <div className="flex flex-col gap-0.5">
          {items.map((item) => (
            <span key={item} className="text-[10px] font-mono text-foreground truncate">
              {item}
            </span>
          ))}
        </div>
...
const DEMO_METADATA = {
  schemas: {
    ...
    PDGM: {
      ...
      tables: {
        PDGM_PERIOD: {
          description: "PDGM payment periods",
          primaryKey: "pp_id",
          columns: [
            { name: "pp_id", type: "int", role: "primary_key" },
            { name: "pp_epi_id", type: "int", role: "foreign_key" },
            { name: "pp_start_date", type: "date", role: "time_dimension" },
            { name: "pp_end_date", type: "date", role: "time_dimension" },
            { name: "pp_payment", type: "decimal", role: "measure", aggregation: "sum" },
            { name: "pp_type", type: "varchar", role: "dimension" },
            { name: "pp_hipps_code", type: "varchar", role: "dimension" },
          ],
          foreignKeys: [
            { column: "pp_epi_id", references: { table: "dbo.CLIENT_EPISODES_ALL", column: "epi_id" } },
          ],
        },
      },
    },
