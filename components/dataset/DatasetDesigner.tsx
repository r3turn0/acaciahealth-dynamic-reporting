          ...
          {visibleDatasets.map((ds) => (
            <div key={ds.datasetId} className="border border-border rounded-xl p-4 flex flex-col gap-3 bg-card hover:border-primary/30 transition-colors">
              ...
              <div className="mt-auto flex flex-wrap gap-2 border-t border-border/50 pt-3">
                ...
                {(ds.status === "Draft" || ds.status === "Pending Approval") && <><button onClick={() => beginEdit(ds)} className="flex items-center gap-1.5 rounded-lg border border-border px-2.5 py-1.5 text-xs text-foreground hover:bg-muted/30"><Edit3 className="size-3" />Edit Draft</button><button type="button" onClick={() => ds.status === "Draft" ? onRequestApproval(ds.datasetId) : onPublish(ds.datasetId)} disabled={ds.status === "Draft" && (ds.health ?? 0) < 70} title={ds.status === "Draft" && (ds.health ?? 0) < 70 ? `Dataset health must reach 70% (current: ${ds.health ?? 0}%)` : undefined} className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-chart-3/30 bg-chart-3/15 px-3 py-1.5 text-xs font-medium text-chart-3 hover:bg-chart-3/25 disabled:cursor-not-allowed disabled:opacity-50"><ShieldCheck className="size-3" />{ds.status === "Draft" ? "Request Approval" : "Certify & Publish"}</button></>}
